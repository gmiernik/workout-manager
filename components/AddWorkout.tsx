...
import { Alert, Button, ButtonProps, StyleSheet, Text, TextInput, View, FlatList, NativeTouchEvent, NativeSyntheticEvent } from 'react-native';
...
export default () => {
    ...
    return (
        <View>
            ...
            <Button title="Dodaj ćwiczenie" onPress={() => addExercise(exerciseType, series, 1.0)} />
        </View>
    );

    function addRepetition(event: NativeSyntheticEvent<NativeTouchEvent>) {
        if (event) {
            event.preventDefault();
        }
        setSeries([...series, parseInt(repetition)]);
        setRepetition("");
    }
}